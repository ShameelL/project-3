...
def get_user_tweets(user):
	if user in CACHE_DICTION:
		...
		user_data = api.user_timeline(user)
	try:
...
for message in umich_tweets:
	tupl = message['user']['id_str'],message['user']['screen_name'],message['user']['favourites_count'],message['user']['description']
	cur.execute('SELECT user_id FROM Users WHERE user_id = ?',(message['user']['id_str'],))
	try:
		acct = cur.fetchone()[0]
	except:
		print('No unretrieved Twitter accounts found')
		cur.execute('INSERT INTO Users (user_id,screen_name,num_favs,description) VALUES (?,?,?,?)',tupl)

for message in umich_tweets:
	for user_mentioned in message['entities']['user_mentions']:
		users_information = api.get_user(user_mentioned['screen_name'])
		tupl = users_information['id_str'],users_information['screen_name'],users_information['favourites_count'],users_information['description']
		cur.execute('SELECT user_id FROM Users WHERE user_id = ?',(user_mentioned['id_str'],))
		try:
			acct = cur.fetchone()[0]
		except:
			print('No unretrieved Twitter accounts found')
...
for message in umich_tweets:
	tupl = message['id_str'],message['text'],message['user']['id_str'],message['created_at'],message['retweet_count']
	cur.execute('SELECT tweet_id FROM Tweets WHERE tweet_id = ?',(message['id_str'],))
	try:
		acct = cur.fetchone()[0]
	except:
		print('No unretrieved tweets found')
...
cur.execute('SELECT screen_name, text FROM Users INNER JOIN Tweets ON Users.user_id = Tweets.user_posted')
joined_data = cur.fetchall()
# Make a query using an INNER JOIN to get a list of tuples with 2
# elements in each tuple: the user screenname and the text of the
# tweet in descending order based on retweets. Save the resulting
# list of tuples in a variable called joined_data2.
cur.execute('SELECT screen_name, text FROM Users INNER JOIN Tweets ON Users.user_id = Tweets.user_posted ORDER BY Tweets.retweets DESC;')
